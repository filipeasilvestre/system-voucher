...
export default function EditVoucherPage() {
  ...
  const { id } = useParams(); // Obtém o ID do voucher da URL
  ...
  // Carrega os dados do voucher existente
  useEffect(() => {
    const fetchVoucher = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setVoucherData({
            ...data,
            amount: data.amount.toString(), // Converte o valor para string
          });
          setSelectedTemplate(data.template || "classic");
        } else {
          console.error("Erro ao carregar voucher:", response.status);
        }
      } catch (error) {
  ...
  const handleSaveVoucher = async () => {
    try {
      ...
      const response = await fetch(`/api/vouchers/${id}`, {
        ...
        body: JSON.stringify({
          ...voucherData,
          amount: parseFloat(voucherData.amount), // Converte para número
        }),
      });
      ...
      if (response.ok) {
        ...
        console.error("Erro ao atualizar voucher");
      }
    } catch (error) {
  ...
  if (isLoading) {
    return <div>Carregando dados do voucher...</div>;
  }
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-800 py-8">
          <div className="container mx-auto px-4">
            ...
            <p className="text-white text-center mt-2">
              Update and personalize your voucher
            </p>
          </div>
        </div>
                  ...
                  <CardContent>
                    <div className="w-full aspect-[3/2] overflow-hidden rounded-lg shadow-lg">
                      ...
                      >
                        ...
                        >
                          {voucherData.showQRCode && voucherData.qrCode && (
                            <img
                              src={
                                voucherData.qrCode ||
